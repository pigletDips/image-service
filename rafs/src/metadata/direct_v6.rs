...
impl OndiskInodeWrapper {
    ...
    fn data_block_mapping(&self, index: usize) -> RafsResult<*const u8> {
        ...
        let r = match layout {
            ...
            //     // let format = (inode.union() & 0x0000_ffff) as u16;
            //     // // let mut unit = 8;
            //     // if format & EROFS_CHUNK_FORMAT_INDEXES_FLAG == 0 {
            //     //     // unit = 4;
            //     //     warn!("fuckme!!!!!!!!!!!!!!!!!!!!wqwqwqwqwqq");
            //     // }
            //     // let chunk_addresses = self.chunk_addresses(index as u32)?;
            //     // unsafe {
            //     //     m.base
            //     //         .add(chunk_addresses[0].block_addr() as usize * EROFS_BLOCK_SIZE as usize)
            //     // }
            //     0
            // }
            _ => {
...
impl RafsInode for OndiskInodeWrapper {
    ...
    fn validate(&self, _inode_count: u64, chunk_size: u64) -> Result<()> {
        ...
        let xattr_size = self.xattr_size();
        ...
        if self.is_reg() {
            ...
            // TODO add other validations for reg file
        } else if self.is_dir() {
            // TODO add validation for dir file
        } else if self.is_symlink() {
            if self.size() == 0 {
                return Err(einval!("invalid symlink target"));
            }
        }
        // TODO add validation for hard link file
        Ok(())
    }
    ...
    fn get_chunk_count(&self) -> u32 {
        div_round_up(self.size(), self.chunk_size() as u64) as u32
    }
    ...
    fn parent(&self) -> u64 {
        match self.parent_inode {
            Some(inode) => inode,
            None => {
                debug_assert!(self.is_dir());
                // # Safety
                // Safe to unwrap, due to all directories has the .. entry
